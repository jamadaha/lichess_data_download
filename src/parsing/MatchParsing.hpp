...
namespace MatchParsing {
    ...
    static uint ParseElo(std::string line) {
        line = ParseGeneric(line);
    ...
    static MatchInfo ParseMatch(std::vector<std::string> lines) {
        ...
        const uint whiteElo = ParseElo(lines[7]);
        const uint blackElo = ParseElo(lines[8]);
        const MatchTermination termination = ParseMatchTermination(lines[14]);
